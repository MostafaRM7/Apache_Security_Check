...
# check.py
# This script checks the configuration for ID: V-2259
...
def check():
    """Checks if the directive Use is set correctly."""
    ...
    match = re.search(r"^\s*Use\s+(\d+)", content, re.MULTILINE)
    
    if match:
        current_value = int(match.group(1))
        if 771 is not None and current_value < 771:
